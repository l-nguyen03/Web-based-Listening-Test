...
$(document).ready(async function () {
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          await setDoc(doc(db, 'position', uid.toString()), {});
          testRef = doc(db, 'position', uid.toString());
          console.log(testRef);
        } else {
    ...
    var testTypes = ["airplane", "car", "pub", "sine", "train"];
    var fileNames = ["65_tot_mismatch.wav", "65_tot.wav","260_tot_mismatch.wav", "260_tot.wav", "1040_tot_mismatch.wav", "1040_tot.wav",]
    ...
    function generateTest() {
        ...
        var testResults = {
            "65_tot_mismatch.wav": 0,
            "65_tot.wav": 0,
            "260_tot_mismatch.wav": 0,
            "260_tot.wav": 0,
            "1040_tot_mismatch.wav": 0,
            "1040_tot.wav": 0
        };
        ...
        if (testTypes.length === 0) {
            ...
            $("a").attr("href", "./part3.html");
        }
        var current = -1;

        var audio0 = new Audio(`../media/pos/${actualTestType}/${actualTestType}_passive.wav`);

        var audio1 = new Audio(`../media/pos/${actualTestType}/${actualTestType}_${fileNames[0]}`);
        $("#1 .slider input").attr("id", fileNames[0]);

        var audio2 = new Audio(`../media/pos/${actualTestType}/${actualTestType}_${fileNames[1]}`);
        $("#2 .slider input").attr("id", fileNames[1]);

        var audio3 = new Audio(`../media/pos/${actualTestType}/${actualTestType}_${fileNames[2]}`);
        $("#3 .slider input").attr("id", fileNames[2]);

        var audio4 = new Audio(`../media/pos/${actualTestType}/${actualTestType}_${fileNames[3]}`);
        $("#4 .slider input").attr("id", fileNames[3]);

        var audio5 = new Audio(`../media/pos/${actualTestType}/${actualTestType}_${fileNames[4]}`);
        $("#5 .slider input").attr("id", fileNames[4]);

        var audio6 = new Audio(`../media/pos/${actualTestType}/${actualTestType}_${fileNames[5]}`);
        ...
        $("#submit").off().on("click", async (e) => {
            ...
            window.location.href = "https://listening-test-pt3.web.app/";
        })
