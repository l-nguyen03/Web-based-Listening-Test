import { db, auth } from "./firebase_pt3.js"
...
$(document).ready( async function () {
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          await setDoc(doc(db, 'slope', uid.toString()), {});
          testRef = doc(db, 'slope', uid.toString());
          console.log(testRef);
        } else {
    ...
    var fileNames = ["high_800Hz.wav", "high_3200Hz.wav","high_12800Hz.wav"]
    ...
    function generateTest() {
        ...
        if (testTypes.length === 0) {
            ...
            $("a").attr("href", "https://listening-test-pt5.web.app/");
        }
        var current = -1;

        var audio0 = new Audio(`../media/slope/${actualTestType}/${actualTestType}_passive.wav`);

        var audio1 = new Audio(`../media/slope/${actualTestType}/${actualTestType}_${fileNames[0]}`);
        $("#1 .slider input").attr("id", fileNames[0]);

        var audio2 = new Audio(`../media/slope/${actualTestType}/${actualTestType}_${fileNames[1]}`);
        $("#2 .slider input").attr("id", fileNames[1]);

        var audio3 = new Audio(`../media/slope/${actualTestType}/${actualTestType}_${fileNames[2]}`);
        ...
        $("#submit").off().on("click", async (e) => {
            ...
            window.location.href = "https://listening-test-pt5.web.app/";
        })
